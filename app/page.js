...
function useWake() {
  ...
  useEffect(() => {
    ...
    async function check() {
      try {
        ...
        if (r.ok && !cancelled) {
          ...
          setProgress(100);
          setTimeout(() => { if (!cancelled) setState("ready"); }, 400);
          return true;
        }
      } catch {}
      return false;
    ...
    async function run() {
      const quick = await check();
      if (quick || cancelled) return;
      setState("waking");
      startProgress();
      while (!cancelled) {
...
function useLiveBrowser(ready) {
  ...
  useEffect(() => {
    ...
    es.onmessage = (e) => {
      // Deduplicate via rAF — only render latest frame per paint cycle
    ...
    es.onerror = () => {
      // SSE auto-reconnects; silence errors
    };
            ...
            {liveFrame ? (
              ...
              <div className="viewport-empty">
                ...
                <span className="empty-label">
                  {isReady ? "tarayıcı bekleniyor" : "başlatılıyor..."}
                </span>
              </div>
            )}
          </div>
      ...
      <style jsx global>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .msgs {
          ...
          scrollbar-color: #14143a transparent;
        }
        ...
        .divider {
          width: 1px;
          background: linear-gradient(to bottom, transparent 5%, #7c5cfc30 40%, #00e5a020 60%, transparent 95%);
          flex-shrink: 0;
        }
        ...
        .grid-bg {
          ...
          background-image:
            linear-gradient(#12123a18 1px, transparent 1px),
            linear-gradient(90deg, #12123a18 1px, transparent 1px);
